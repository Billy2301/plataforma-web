...
{
    public class GenericRepositoryClinica<TEntity> : IGenericRepositoryClinica<TEntity> where TEntity : class
    {
        public readonly CPALClinicaContext _dbContext;
        public GenericRepositoryClinica(CPALClinicaContext dbContext)
        {
        ...
        public async Task<TEntity?> Obtener(Expression<Func<TEntity, bool>> filtro, Func<IQueryable<TEntity>, IQueryable<TEntity>>? include = null)
        {
            try
            {
                ...
                TEntity? entidad = await query.FirstOrDefaultAsync(filtro);
                return entidad;
            }
            catch (Exception)
            {
        ...
        public async Task ExecuteQuery(string procedure)
        {
